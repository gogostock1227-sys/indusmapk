// /api/data/master-patch
//   GET → concept_taxonomy/master_patch_v3.json
//   PUT → 整檔覆蓋
...
const FILE_PATH = "concept_taxonomy/master_patch_v3.json";

export const onRequestGet = async (ctx: RequestCtx) => {
  const guard = requireDataPerm(ctx.data.user, "master_patch");
  if (guard) return guard;
  try {
    const file = await getFile(ctx.env, FILE_PATH);
    if (!file) return jsonError("master_patch_v3.json 不存在", 404);
...
export const onRequestPut = async (ctx: RequestCtx) => {
  ...
  const guard = requireDataPerm(caller, "master_patch");
  if (guard) return guard;
  ...
  const message = `data: 編輯 master_patch (${Object.keys(data).length} 族群) by ${caller.email}`
    + (summary ? `\n\n${summary}` : "");
