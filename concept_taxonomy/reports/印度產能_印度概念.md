...
- 2301 光寶科 `pos=PASSIVE` （信心 0.75；產業板塊匹配 (COMP_HW)；必要題材交集：[]；位階「PASSIVE」不在白名單（族群 allowed=['ODM...）
...
| 代號 | 公司 | 移除原因 | 信心 |
|---|---|---|---:|
| 2408 | 南亞科 | 產業板塊不匹配（個股 AI_SEMI vs 族群允許 ['COMP_HW', 'ELEC_COMP', 'CONSUMER']）；必要題材交集：[]；位階「ID | 0.50 |
